...
func (this *MongoJobStore) Create(item JobDetails, tasks []Task) os.Error {
	...
	jobsCollection, err := this.GetCollection(this.JOBS)
	if err != nil {
		return err
	}
	...
	tasksCollection, err := this.GetCollection(this.TASKS)
	if err != nil {
		return err
	}
